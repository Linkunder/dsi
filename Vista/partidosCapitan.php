...
// Traigo la info del partido 
$partido = $jefePartidos->leerPartido($idPartido);

foreach ($partido as $key) {
  $fecha = $key->getFecha();
  $hora = $key->getHora();
  $idRecinto = $key->getIdRecinto();
  $idTercerTiempo = $key->getIdTercerTiempo();
  $idCapitan = $key->getIdUsuario();
}

//echo $idTercerTiempo;

if ($idTercerTiempo != 0){ // Hay Tercer tiempo.
  $vectorTercerTiempo = $jefeTercerTiempo->leerTercertiempo($idTercerTiempo);
  foreach ($vectorTercerTiempo as $key ) {
    $horaTercer = $key->getHora();
    $descripcion = $key->getDescripcion();
    $idLocal = $key->getIdLocal();
  }
  $vectorLocales = $jefeLocal->leerLocal($idLocal);
} 
// Usuarios 
...
<div id="contact-nosotros" class="parallax">
  <div class="container">
     <?php
      if ($idTercerTiempo==0){ // SI NO HAY TERCER TIEMPO 
        ?>
    <div class="row">
      <h2>Resumen del partido</h2> 
            <div>
        <center>
          <a href="../Logica/eliminarPartido.php">
            <button class="btn btn-lg btn-primary"> <i class="fa fa-cross-left" aria-hidden="true"></i> Eliminar </button>
          </a>
        </center>
        <br/>
      </div>
